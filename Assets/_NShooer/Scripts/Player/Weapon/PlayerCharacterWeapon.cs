...
namespace NShooter 
{
	public class PlayerCharacterWeapon : NetworkBehaviour
	{
        ...
        [Header("Fire Config")]
		[SerializeField] float _fireRate = 0.1f;		// 射速
		[SerializeField] int _maxAmmo = 30;				// 弹夹容量
		[SerializeField] int _damage = 20;				// 武器伤害
		public float _reloadDuration = 2f;						// 换弹速度
		[SerializeField] float _maxDistance = 100;		// 射程	
		[SerializeField] Transform _muzzlePoint;		// 枪口位置
		[SerializeField] LayerMask _hitLayerMask;		// 击中Layer
		float _lastShotTimeLocal; 	// 客户端本地记录（仅用于输入控制）

		[Header("Server Data")]
		[SerializeField] float _lastShotTime;			// 上次射击时间（服务器维护）
		[SerializeField] float _shotTimeTolerance = 0.02f;// 射击间隔波动允许（服务器维护）

		[Header("Sync Data")]
		[SyncVar(hook = nameof(OnAmmoChanged))]
		public int _currentAmmoAmount;							// 当前子弹数
		[SyncVar]
		bool _isReloading = false;						// 当前是否在装弹
		...
		WaitForSeconds _cachedReloadWait;
        ...
        BulletData calcBulletRayCast(out bool isHit, out RaycastHit hitInfo)
		{
			isHit = false;
